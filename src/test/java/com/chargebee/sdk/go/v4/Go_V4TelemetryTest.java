...
@DisplayName("Go_V4 Telemetry Generator")
class Go_V4TelemetryTest {
  ...
  @Test
  @DisplayName("Should pass resource and operation metadata from generated services")
  void shouldPassResourceAndOperationMetadata() throws IOException {
    List<FileOp> fileOps = generate();

    FileOp.WriteString services = findWriteOp(fileOps, "/customer_service.go");
    assertThat(services.fileContent).contains("req.telemetryResource = \"customer\"");
    assertThat(services.fileContent).contains("req.telemetryOperation = \"retrieve\"");
  }
}
