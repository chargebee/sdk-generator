...
public class NodeV3Tests extends LanguageTests {
  ...
  void shouldGenerateTelemetryModule() throws IOException {
    ...
    assertThat(typesFile)
        ...
        .contains("responseHeaders?: ResponseHeadersForTelemetry;")
        .contains("endAttributes: Record<string, RequestTelemetryEndAttributeValue>;")
        .contains("requestHeaders?: Record<string, string | number>;");

    var parserFile =
        assertTelemetryWriteFileOp(
            fileOps.get(6), "/node/lib/telemetry", "chargebeeTelemetryHeaderParser.ts");
    assertThat(parserFile)
        .contains("export function parseChargebeeTelemetryHeaderToSpanAttributes(")
        .contains("export function parseScalarValue(");

    var adapterFile =
        assertTelemetryWriteFileOp(fileOps.get(7), "/node/lib/telemetry", "TelemetryAdapter.ts");
    assertThat(adapterFile)
        .contains("export function buildRequestHeaderSpanAttributes(")
        .contains("if (name == null || value === undefined || value === null)")
        .contains("export function getResponseHeaderValueIgnoreCase(")
        .contains("export function buildResponseHeaderSpanAttributes(")
        .contains("parseChargebeeTelemetryHeaderToSpanAttributes(value)")
        .contains("...buildResponseHeaderSpanAttributes(result.responseHeaders),")
        .contains("export function extractResponseHeaders(")
        .contains("...buildRequestHeaderSpanAttributes(input.requestHeaders),");

    var indexFile = assertTelemetryWriteFileOp(fileOps.get(8), "/node/lib/telemetry", "index.ts");
    assertThat(indexFile)
        .contains("CHARGEBEE_SDK_NAME,")
        .contains("HTTP_RESPONSE_HEADER_ATTRIBUTE_PREFIX,")
        .contains("parseChargebeeTelemetryHeaderToSpanAttributes,")
        .contains("buildResponseHeaderSpanAttributes,")
        .contains("extractResponseHeaders,")
        .contains("buildRequestHeaderSpanAttributes,")
        .contains("getResponseHeaderValueIgnoreCase,");

    assertTelemetryWriteFileOp(fileOps.get(9), "/node/lib/telemetry", "otel.ts");
  }
