...
public class ChangeLogGenerator implements FileGenerator {
    ...
    public FileOp generate(String output, Spec oldVersion,Spec newerVersion) throws IOException {
        ...
        changeLogSchema.setNewEventType(generateEventLine(oldVersion.extractWebhookInfo(false), newerVersion.extractWebhookInfo(false)));
    ...
    private List<String> generateEventLine(List<Map<String, String>> oldEvents,
                                           List<Map<String, String>> newEvents) {
        Set<String> oldEventTypes = oldEvents.stream()
    ...
    private String convertNewActionToActionLine(Resource resource, Action action) {
        ...
        return String.format("- [%s](%s) has been added to [%s](%s).", action.id, getDocsUrlForActions(resource, action), resourceName, getDocsUrlForResource(resource));
    }
